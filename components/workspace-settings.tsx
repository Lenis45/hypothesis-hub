...
import { ArrowRightLeft, Flag, FolderPlus, GitBranch, Plus, Scale, Target } from "lucide-react";
...
export function WorkspaceSettings({ activeWorkspace, projects, objectives, stages, scales }: { activeWorkspace: Project; projects: Project[]; objectives: Objective[]; stages: Stage[]; scales: Scales }) {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 px-4 py-6 lg:px-8">
    ...
    <section id="funnel" className="grid gap-5 lg:grid-cols-[1fr_.95fr]">
      <div className="card"><div className="flex items-start gap-3"><GitBranch className="mt-1 text-[#d3f96a]" size={20}/><div><p className="eyebrow">Воронка</p><h2 className="mt-1 text-xl font-bold">Этапы пути пользователя</h2><p className="mt-1 text-sm text-zinc-500">Новые этапы добавляются в конец. Они сразу появятся при создании гипотезы.</p></div></div><ol className="mt-5 space-y-2">{stages.length ? stages.map((stage, index) => <li key={stage.id} className="flex items-center gap-3 rounded-lg border border-white/10 bg-white/[0.025] p-3"><span className="grid h-6 w-6 place-items-center rounded-full bg-white/[0.07] text-xs font-bold text-[#d3f96a]">{index + 1}</span><span className="font-medium">{stage.name}</span></li>) : <li className="rounded-lg border border-dashed border-white/15 p-4 text-sm text-zinc-500">Воронка пока пуста. Добавьте первый этап — например, «Просмотр лендинга».</li>}</ol></div>
